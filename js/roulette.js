...
{
  document.getElementById('betto').style.display="block";
  document.getElementById('betname').innerHTML=name;
  currentbet=id;
}
function timerfunc()
{
  lefttime--;
  if (lefttime<=0) top.frames["TOP"].location="roulette.php";
  leftsec=lefttime%60;
  leftmin=Math.floor(lefttime/60);
  if (leftsec<10) leftsec="0"+leftsec;
  document.getElementById('timercl').innerHTML=leftmin+":"+leftsec;
...
function betclick()
{
  window.location.href='roulette.php?bet='+document.getElementById('bet').value+'&betto='+currentbet;
}
